# 人体功能统一本体 UOHF V1.0——中文历史版本

[English](README.md) | [中文](README.zh-CN.md) | [版本档案](../README.md)

> **历史版本说明：** UOHF V1.0 是人体功能统一本体正式发展历史的一部分，但不是当前权威根定义。当前框架请参见 [UOHF Definition 2.1](../../UOHF_DEFINITION_ZH.md)。
...
UOHF V1.0 首次系统提出一套本体驱动、治理导向、安全约束和可审计的人体功能推理框架，主要面向康复与运动训练场景中那些可能尚未形成明确疾病诊断、但已经表现为疼痛、代偿、耐受下降、表现不稳定和风险升高的人体功能问题。
...
这一表述属于 V1.0 的历史运行框架，为保证版本真实性予以保留。当前 UOHF Definition 2.1 的权威根定义为：
...
对应八项因素：任务、动作、策略、能力、系统、结构、心理和行为。
...
其中 `C` 表示能力供给向量，`D` 表示任务需求向量，`M` 表示功能裕度向量。

## 最小可行状态空间

V1.0 建立了早期 **MVSS-12** 运行表达，包含十二项归一化维度：恢复储备、敏化代理指标、自主神经负荷代理指标、动作控制精度、本体感觉与感觉质量、保护策略强度、可用活动窗口、刚度调节、负荷分配效率、任务能力、协调与节律、代偿依赖程度。
...
V1.0 把安全约束作为硬门控。完整推理工件应当保留本体版本与参数版本、输入快照、已应用约束与门控、裕度与风险估计、候选归因与证据链接、优先决策杠杆、允许行动模板及确定性回放所需来源信息。

## 与后续版本的关系

- **V1.0** 建立了早期确定性、任务—能力、安全门控和可审计的运行方向；
- **Definition 2.0** 从因素与评分中心进一步进入任务中心本体架构、Functional Engagement、持续回写以及 UOHF—IHFM—HFWM 分层；
- **Definition 2.1** 成为当前权威框架定义，恢复内部需求与外部需求的完整范围，并明确多尺度本体与人体功能世界模型的发展路径。

V1.0 具有重要历史价值，对特定的任务—能力匹配应用仍有参考意义，但不应被视为当前 UOHF 完整的上层本体架构。
